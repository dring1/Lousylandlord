...
(function() {
  jQuery(function() {
    $('.address-form').hide();
    return $('#address-form-toggle').click(function() {
      return $('.address-form').toggle();
    });
  });
...
(function() {
  jQuery(function() {
    ...
    return $('#landlord_province_id').change(function() {
      var escaped_province, options, province;
      ...
      if (options) {
        ...
        return $('#landlord_city_id').parent().show();
      } else {
        $('#landlord_city_id').empty();
        return $('#landlord_city_id').parent().hide();
      }
    });
  });
...
(function() {
  var blueIconLink, createInfoWindow, displayAllMarkers, encodeAddress, geocoder, gm_init, infowindows, lastWindow, map, root;
  ...
  gm_init = function() {
    ...
    return map = new google.maps.Map(this.map_canvas, map_options);
  };
  ...
  encodeAddress = function(address) {
    return geocoder.geocode({
      ...
      if (status === google.maps.GeocoderStatus.OK) {
        ...
        return alert("Geocoder could not find the location");
      }
    });
  };

  displayAllMarkers = function() {
    var bounds, coord, i, landlord, marker, _i, _len, _ref, _results;
    if (typeof gon === "undefined" || gon === null) {
      return;
    }
    i = 0;
    ...
    while (i < gon.addresses.length) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (gon.addresses[i].landlord_id === landlord.id) {
          createInfoWindow(marker, "<div class=\"infowindow\" style=\"width:250px\">" + "<b>" + landlord.name + "</b><br>" + gon.addresses[i].number + " " + gon.addresses[i].street + ", " + gon.city + "<br/>" + "<a href=\'/landlords/" + landlord.id + "\'>View " + landlord.name + "\'s page</a><br/>" + "</div>");
        }
      }
      i++;
  ...
  root.centerMapOnCoordinates = function(lat, long) {
    return map.setCenter(new google.maps.LatLng(lat, long));
  };

  root.displayLandmarkMarker = function() {
    var address;
    ...
    return geocoder.geocode({
      ...
      if (status === google.maps.GeocoderStatus.OK) {
        ...
        return createInfoWindow(marker, "<b>" + address + "</b>");
      } else {
        return alert("Geocoder could not find the location");
      }
    });
...
(function() {
  jQuery(function() {
    ...
    return $('#search_province_id').change(function() {
      var escaped_province, options, province;
      ...
      if (options) {
        ...
        return $('#search_city_id').parent().show();
      } else {
        $('#search_city_id').empty();
        return $('#search_city_id').parent().hide();
      }
    });
  });
